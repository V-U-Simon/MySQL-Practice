...
CREATE TABLE `profiles` (
    ...
    hometown VARCHAR(100) -- , FOREIGN KEY (photo_id) REFERENCES media(id) -- пока рано, т.к. таблицы media еще нет
);
...
CREATE TABLE communities(
    ...
    FOREIGN KEY (admin_user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE
    SET
        NULL
);
...
CREATE TABLE media_types(
    ...
    name VARCHAR(255),
    created_at DATETIME DEFAULT NOW(),
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP -- записей мало, поэтому индекс будет лишним (замедлит работу)!
);
...
ALTER TABLE
    `profiles`
ADD
    CONSTRAINT fk_photo_id FOREIGN KEY (photo_id) REFERENCES photos(id) ON UPDATE CASCADE ON DELETE
set
    NULL;