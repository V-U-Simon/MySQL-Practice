...
FROM
  ...
  JOIN friend_requests ON (
    users.id = friend_requests.initiator_user_id
    OR users.id = friend_requests.target_user_id
  )
WHERE
  friend_requests.status = 'approved'
GROUP BY
  users.id;
...
FROM
  media m
  JOIN friend_requests fr ON (
    fr.initiator_user_id = m.user_id
    AND fr.target_user_id = 1
  )
  OR (
    fr.target_user_id = m.user_id
    AND fr.initiator_user_id = 1
  )
WHERE
  fr.status = 'approved';
...
GROUP BY
  u.id
ORDER BY
  total_communities DESC;

-- Среднее количество групп у всех пользователей    
SELECT
  AVG(total_communities) AS average_communities
FROM
  (
    SELECT
      u.firstname,
